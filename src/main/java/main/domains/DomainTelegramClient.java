...
public abstract class DomainTelegramClient implements DefaultWebhookBot {
	@AllArgsConstructor
	@Getter
	...
	private final YandexDNSClient dns;
		...
		return switch (chatState.currentState.get()) {
			...
			case WAIT_DOMAIN_NAME_TO_ADD -> {
				if (isValidDomain(messageText)) {
					...
					boolean add = dns.addSubdomain(rr);
					if (add) {
			...
			case WAIT_DOMAIN_NAME_TO_DELETE -> {
				if (isValidDomain(messageText)) {
					...
					boolean delete = dns.deleteSubdomain(rr);
					if (delete) {
						chatState.currentState.set(NOTHING_WAIT);
						yield send("::ok::\n/list");
					} else {
	...
	private BotApiMethod<?> handleCommand(String messageText) {
		return switch (commandsMap.get(messageText)) {
			case START -> send("Тут можно управлять поддоменами домена " + dns.getDomainName());
			case LIST -> sendList();
			case ADD -> {
